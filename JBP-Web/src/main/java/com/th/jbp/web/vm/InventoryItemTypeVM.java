...
public class InventoryItemTypeVM extends BaseVM{
	...
	private void loadItems() {
		...
		Page<InventoryItemTypeM> page = inventoryItemTypeService.lists(name, prefix, pageRequest);
	...
	public void getByItem(@BindingParam("item") InventoryItemTypeM inventoryItemTypeM) {
		...
		this.inventoryItemTypeM = inventoryItemTypeService.getByKey(inventoryItemTypeM).getObject();
	}
	...
	public void delete(@BindingParam("item") final InventoryItemTypeM inventoryItemTypeM) {
		...
		execute(new MethodOperation() {
			...
			public void execute() {
				...
				try{
					ObjectResult<InventoryItemTypeM> objectResult = inventoryItemTypeService.delete(inventoryItemTypeM);
		...
		execute(new MethodOperation() {
			...
			public void execute() {
				...
				try {
					LOGGER.info(model);
					if (isModeAdd) {
						inventoryItemTypeService.save(model);
					} else {
						inventoryItemTypeService.update(model);
					}
