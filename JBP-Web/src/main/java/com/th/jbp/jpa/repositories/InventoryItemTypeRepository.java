...
public interface InventoryItemTypeRepository extends JpaRepository<InventoryItemTypeM, Long>{
	...
	@Query("SELECT b FROM InventoryItemTypeM b WHERE upper(b.name) = upper(:name) and b.status.name != 'DELETED' order by b.name")
	List<InventoryItemTypeM> findActiveByName(@Param("name") String name);
